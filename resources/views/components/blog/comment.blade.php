				...
				<div class="row">
					...
					<div class="col-lg-10 col-9 my-auto">
						...
						<p class="mb-1 small fw-normal text-secondary">
							...
							<a href="#write-comment" class="reply_btn small" data-reply_name="{{ $comment->name }}" data-parent="{{ $comment?->blog_comment_id ? $comment?->blog_comment_id : $comment->id }}"><i class="fas fa-share"></i> Reply</a>
						</p>
						<p class="mb-0 small">
							@if($comment->reply_to_name)
								<a href="#comment_{{ $comment->blog_comment_id }}" class="badge border text-primary text-decoration-none">
